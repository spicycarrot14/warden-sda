...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(()=>{
    ...
    if(t) { t.classList.add('vis'); appState = 'L1'; }
  }, 3000);
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const mt = document.getElementById('mon-text');
  if(mt) mt.textContent = '1 MONITORED';
});
...
function selectCOA(id){
  ...
  activeCOAPreview = id;
  if(window.zoomToVehicle && !zoomTarget) window.zoomToVehicle('usa342');
...
// ── SEARCH ──
let SEARCHABLE = [];
document.addEventListener("DOMContentLoaded", () => { if(typeof VEH_DATA !== "undefined") SEARCHABLE = Object.entries(VEH_DATA).map(([id,d])=>({id,label:d.label,type:d.type})); });
